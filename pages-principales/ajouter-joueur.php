<?php
// pages-principales/ajouter-joueur.php
...
// Vérifier si l'entraîneur est connecté, sinon rediriger vers la connexion
Authentification::verifierConnexion();

// Définir le titre et la description de la page
$titrePage = 'Ajouter un Joueur';     //Titre de la page
$descriptionPage = 'Ajouter un nouveau joueur à l\'équipe';   //Description de la page
...
$messageErreur = '';                // Message d'erreur (vide au départ aussi)
$donneesFormulaire = [              // Données du formulaire avec valeurs par défaut
    'nom_joueur' => '',             
    'prenom_joueur' => '',
    'numero_licence' => '',
    'date_naissance' => '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {    // Si le formulaire a été soumis (avec la méthode POST)
    
    // écrire dans le log que l'ajout d'un joueur commence
    error_log("=== DÉBUT AJOUT JOUEUR ===");    

    // on récupère et nettoie les données du formulaire
    $donneesFormulaire = [
        'nom_joueur' => trim($_POST['nom_joueur'] ?? ''),   //// trim() enlève les espaces inutiles
    ...
    // on affiche dans le log les données reçues 
    error_log("Données formulaire reçues: " . print_r($donneesFormulaire, true));

    // on initialise un tableau pour collecter les erreurs de validation
    $erreurs = [];
    ...
    if (empty($donneesFormulaire['taille_cm'])) {
        ...
        // vérifier que la taille est un nombre valide entre 100 et 250 cm
        // on remplace la virgule par un point pour gérer les décimales
    ...
    if (empty($donneesFormulaire['poids_kg'])) {
        ...
        // vérifier que le poids est un nombre valide entre 30 et 150 kg
    ...
    // si pas d'erreurs, procéder à l'insertion dans la BDD
    if (empty($erreurs)) {
        try {
            // dans le log : validation réussie
            error_log("Validation réussie, vérification licence...");   

            // Vérifier l'existence du numéro de licence
            $licenceExiste = $fonctionsBDD->verifierLicenceExiste($donneesFormulaire['numero_licence']);
            error_log("Vérification licence: " . ($licenceExiste ? 'existe' : 'n\'existe pas'));

            // si la licence existe, afficher un message d'erreur
            if ($licenceExiste) {
                $messageErreur = 'Ce numéro de licence existe déjà';
            } else {
                // Préparer les données d'insertion (avec placeholders PDO)
                $donneesInsertion = [
                ...
                // appel de la méthode d'insertion en BDD
                $idJoueur = $fonctionsBDD->insererJoueur($donneesInsertion);
                //log du resultat de l'insertion
                error_log("Résultat insertion: " . var_export($idJoueur, true));

                // si l'insertion OK
                if ($idJoueur) {
                    // on affiche un message de confirmation
                    $messageSucces = 'Le joueur a été ajouté avec succès!';

                    // et on réinitialise le formulaire en vidant tous les champs
                    $donneesFormulaire = [
                        'nom_joueur' => '',
                        'prenom_joueur' => '',
                        'numero_licence' => '',
                    ...
                    // Redirection automatique vers la page gestion-joueurs après 2 secondes                    
                    echo '<script>
                        setTimeout(function() {
                    ...
                    $messageErreur = 'Erreur lors de l\'ajout du joueur. Veuillez réessayer.';
                }
            }
        } catch (Exception $e) {
            // en cas d'erreur technique, afficher le message d'erreur
    ...
    error_log("=== FIN AJOUT JOUEUR ===");
}

// limites date de naissance (16 à 50 ans)
...
<div class="conteneur-formulaire">
    ...
    <div class="carte-formulaire">
        <div class="entete-formulaire">
            <h2><i class="fas fa-user-plus"></i> Ajouter un nouveau joueur</h2>
